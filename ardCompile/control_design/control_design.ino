...
struct Settings
{
  bool states[NUM_COMMANDS] = {false, false, false, false, false, false, false};
  char commands[NUM_COMMANDS][MAX_STRING_SIZE] = {"ledon", "stream", "sample", "debug", "log", "sendlog", "meas"};
...
float control(float y, float x)
{
  e[0] = y - x;
  // One pole transfer function
  // This is actually really good!!
  u[0] = 0.9998*u[1] + 0.01*e[0] - 0.01 * e[1];
  
  // Two pole transfer function
  //u[0] = 1.909*u[1] - 0.9122*u[2] + 0.02*e[1] + 0.02*e[2];
  
  // One pole transfer function
  //u[0] = e[0]*55.4017 - 53.4017*e[1] - u[1];
  //u[0] = 0.9 * u[1] + 0.1 * u[0];
...
void determineTouch(unsigned int pin, int num_samples, bool data_stream, Touch *touch_data)
{
  ...
  if (touch_data->touch_value > 0.05 or touch_data->touch_value < -0.05)
  {
