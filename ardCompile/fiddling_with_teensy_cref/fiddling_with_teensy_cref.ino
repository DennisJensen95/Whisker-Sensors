...
double getTouch(unsigned int pin, int num_samples, int last_value, bool stream, Touch *touch_data)
{
  char id_string[] = "Pin";
  double v = 0;
  double avg = 0;
  ...
  if (pin < MAX_PIN_NUM)
  {
    ...
    // Ignore an offset of one.
    /*
    if (avg + 1 == last_value || avg - 1 == last_value)
    {
      step_value = avg;
      avg = last_value;
...
void determineTouch(unsigned int pin, int num_samples, bool data_stream, int *last_value, Touch *touch_data)
{
  ...
  if (touch_data->touch_value > 2 or touch_data->touch_value < 0)
  {
